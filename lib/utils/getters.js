...
export const getCountDown = (startDate, startTime) => {
  const workingStart = new Date(`${startDate} ${startTime}`)  
  const start = workingStart.getTime()
  const now = new Date().getTime()
  const diff = start - now
  return {
    diff: diff,
    seconds: (diff / 1000).toFixed(1),
    minutes: (diff / (1000 * 60)).toFixed(1),
    hours: (diff / (1000 * 60 * 60)).toFixed(1),
    days: (diff / (1000 * 60 * 60 * 24)).toFixed(1)
  }
}